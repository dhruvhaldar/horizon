...
## 2026-08-20 - Accessible Native SVG Tooltips
**Learning:** For SVG data visualizations (like D3 network graphs), complex HTML-based tooltips often clutter the DOM and introduce complicated interaction and accessibility challenges. Native SVG `<title>` tags applied directly to visual elements (such as `<path>` or `<line>`) provide zero-clutter, accessible tooltips that supply on-demand contextual data natively supported by browsers and screen readers.
**Action:** When implementing SVG data visualizations (like D3 or NetworkX visual outputs), append a native `<title>` element to the core graphical elements (e.g., links or nodes) to provide contextual, accessible information on hover without the overhead of custom tooltip logic.
