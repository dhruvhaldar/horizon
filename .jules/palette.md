...
## 2024-08-20 - SVG Native Tooltips for Data Density
**Learning:** Visualizations like network graphs or route maps often lack labels for edges (like routing probabilities or distances) to avoid visual clutter. This forces users to cross-reference the diagram with raw data tables or inputs to understand the relationships, breaking cognitive flow and reducing the usefulness of the visualization.
**Action:** Always utilize the native SVG `<title>` element appended to `<path>` or `<line>` graphical elements. This creates an accessible, zero-clutter tooltip that provides on-demand contextual data (e.g., "Distance/Cost: 15") precisely where the user focuses, maintaining visual cleanliness while improving data discoverability.
