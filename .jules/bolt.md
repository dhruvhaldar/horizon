...
## 2024-05-15 - Native Kahn's Algorithm vs NetworkX
**Learning:** Instantiating `networkx.DiGraph()` and adding nodes/edges iteratively for thousands of jobs incurs massive overhead due to dictionary wrapping and internal validation. Using native Python dictionaries for adjacency lists (`successors`, `predecessors`, `in_degree`) and implementing Kahn's algorithm directly reduces execution time by roughly 75% for large dense graphs.
**Action:** For performance-critical directed acyclic graphs that only require simple topological sorting or single-pass traversal (like CPM), implement native Python adjacency lists instead of relying on the heavy NetworkX dependency.
