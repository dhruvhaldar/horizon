...
## 2026-07-29 - Python iteration optimization with zip
**Learning:** When iterating over multiple lists in a `for` loop, using `enumerate(zip(list_a, list_b, list_c))` is significantly faster than using a `for i in range(len(list_a)):` loop and manual index lookups (e.g., `list_a[i]`, `list_b[i]`). Manual lookups incur Python index bounds checking overhead on every element, whereas `zip` pushes the iteration to C. Combining `.tolist()` on NumPy arrays with `zip` is the fastest way to process NumPy vectors element-by-element in a Python loop.
**Action:** Use `zip` for concurrent iteration over multiple iterables instead of manual index lookups. When iterating over NumPy arrays element-by-element, first convert them to native Python lists using `.tolist()` and then use `zip`.
