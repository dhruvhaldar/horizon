...
## 2026-11-04 - Physical Metaphors for Validation Feedback
**Learning:** In highly stylized or "tactile" interfaces (like Dark Neumorphism), purely visual error styling (e.g., a dashed red border) might not grab a user's attention effectively because it fights with the prominent shadows and highlights of the base components. Furthermore, visual-only feedback lacks the "physicality" of the UI aesthetic. Adding a physical "shake" animation to invalid inputs on submission directly bridges this gap by mimicking the physical resistance of a broken or stuck knob/slider.
**Action:** When implementing form validation in tactile UIs, pair visual error styling with a brief horizontal shake animation (`@keyframes`) triggered via JavaScript when validation fails. Always remove the shake class immediately upon the next user input so it can be retriggered. Crucially, always wrap the CSS animation in `@media (prefers-reduced-motion: no-preference)` to ensure it does not negatively impact users with vestibular disorders.
