...
async function withLoading(btnElement, asyncFunc) {
    ...
    if (container) {
        ...
        for (let input of visibleInputs) {
            ...
            if (!input.checkValidity()) {
                ...
                if (errorDiv) {
                    const existingDescribedBy = input.getAttribute('aria-describedby') || '';
                    const newDescribedBy = existingDescribedBy.replace(errorDiv.id, '').replace(/\s+/g, ' ').trim();
                    if (newDescribedBy) {
    ...
    btnElement.setAttribute('title', 'Calculating...');
    ...
    try {
        ...
        if (panel) {
            ...
            const calcBtns = panel.querySelectorAll('button[id^="btn-solve-"]');
            calcBtns.forEach(btn => btn.classList.remove('needs-recalc'));
...
document.addEventListener('input', (e) => {
    ...
    if (e.target.hasAttribute('aria-invalid')) {
        ...
        if (e.target.checkValidity()) {
            ...
            if (errorDiv) {
                ...
                const newDescribedBy = existingDescribedBy.replace(errorDiv.id, '').replace(/\s+/g, ' ').trim();
                if (newDescribedBy) {
                    e.target.setAttribute('aria-describedby', newDescribedBy);
                } else {
    ...
    if (panel) {
        ...
        const calcBtns = Array.from(panel.querySelectorAll('button[id^="btn-solve-"]'));
        const calcBtn = calcBtns.find(b => b.offsetParent !== null);
