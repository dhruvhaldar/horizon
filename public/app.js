...
function getCustomError(input) {
  ...
  const label =
    input.labels && input.labels.length > 0 ? input.labels[0] : null;
  if (label) {
...
async function withLoading(btnElement, asyncFunc) {
    ...
    for (let input of visibleInputs) {
      ...
      if (!input.checkValidity()) {
        ...
        if (!existingDescribedBy.includes(errorDiv.id)) {
          input.setAttribute(
            "aria-describedby",
            `${existingDescribedBy} ${errorDiv.id}`.trim(),
          );
        }
        ...
        if (errorDiv) {
          ...
          input.setAttribute(
            "aria-describedby",
            existingDescribedBy.replace(errorDiv.id, "").trim(),
          );
          errorDiv.remove();
        }
      }
  ...
  try {
    ...
    if (panel) {
      ...
      const staleContainers = panel.querySelectorAll(
        ".results, .viz-container",
      );
      staleContainers.forEach((el) => {
        el.style.opacity = "";
        el.style.filter = "";
        if (
          el.classList.contains("viz-container") &&
          ...
          if (currentLabel.startsWith("Out of date: ")) {
            el.setAttribute(
              "aria-label",
              currentLabel.replace("Out of date: ", ""),
            );
          }
        }
      });
...
async function solveQueue() {
  ...
  try {
    ...
    document
      .getElementById("queue-graph")
      .setAttribute(
        "aria-label",
        "Queueing Network Graph (Empty: Run network solver to generate graph)",
      );
...
function drawQueueGraph(gamma, p) {
  ...
  node.append("circle").attr("r", (d) => (d.isExternal ? 5 : 15));
  ...
  document
    ...
    .setAttribute(
      "aria-label",
      `Queueing Network Graph showing ${nodes.length} nodes and ${links.length} connections.`,
    );
}
...
async function solveEOQ() {
  ...
  try {
    ...
    if (viz) {
      ...
      viz.setAttribute(
        "aria-label",
        "Inventory Optimization Chart (Empty: Calculate inventory policy to generate chart)",
      );
    }
  }
}
...
function drawInventoryChart(Q, R, demand) {
  ...
  if (!canvas) {
    viz.innerHTML =
      '<canvas id="inventory-chart" aria-hidden="true">Your browser does not support the canvas element. This chart displays inventory levels over time.</canvas>';
  ...
  if (R > 0) {
    viz.setAttribute(
      "aria-label",
      `Line chart showing inventory levels and reorder points over time. Order Quantity (Q) of ${Q.toFixed(2)} and Reorder Point (R) of ${R.toFixed(2)}`,
    );
  } else {
    viz.setAttribute(
      "aria-label",
      `Line chart showing inventory levels over time. Economic Order Quantity (Q) of ${Q.toFixed(2)}`,
    );
  }
}
...
async function solveTSP() {
  ...
  try {
    ...
    document
      .getElementById("routing-graph")
      .setAttribute(
        "aria-label",
        "Transport Routing Graph (Empty: Optimize route to generate map)",
      );
...
function drawRoutingGraph(nodesList, edges, path) {
  ...
  const pathText =
    path && path.length > 0
      ? ` with optimal route of ${path.length - 1} steps`
      : "";
  document
    .getElementById("routing-graph")
    .setAttribute(
      "aria-label",
      `Transport Routing Graph showing ${nodes.length} nodes and ${links.length} possible connections${pathText}.`,
    );
}
...
document.addEventListener("input", (e) => {
  ...
  if (panel) {
    ...
    staleContainers.forEach((container) => {
      ...
      if (
        container.classList.contains("results") &&
        container.textContent.includes("Results will appear here...")
      )
        return;
      ...
      if (
        container.classList.contains("viz-container") &&
      ...
      if (container.classList.contains("results")) {
        ...
        if (copyBtn && copyBtn.getAttribute("aria-disabled") !== "true") {
          ...
          copyBtn.setAttribute(
            "title",
            "Results are out of date. Recalculate to copy.",
          );
...
document.querySelectorAll(".results").forEach((container) => {
  ...
  const observer = new MutationObserver(() => {
    ...
    if (
      text.includes("Results will appear here...") ||
      text.includes("❌ Error:")
    )
      return;
